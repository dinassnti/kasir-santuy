            ...
            <form action="{{ route('diskon.update', $diskon->id_diskon) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('jenis_diskon') is-invalid @enderror" id="jenis_diskon" name="jenis_diskon" onchange="toggleDiskonInput()" required>
                        ...
                        <option value="persentase" {{ old('jenis_diskon', $diskon->persentase ? 'persentase' : 'nominal') == 'persentase' ? 'selected' : '' }}>Persentase</option>
                        <option value="nominal" {{ old('jenis_diskon', $diskon->nominal ? 'nominal' : 'persentase') == 'nominal' ? 'selected' : '' }}>Nominal</option>
                    </select>
                    @error('jenis_diskon')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3 hidden" id="persentase_field">
                    ...
                    <input type="number" class="form-control @error('persentase') is-invalid @enderror" id="persentase" name="persentase" value="{{ old('persentase', $diskon->persentase) }}" step="0.01" min="0" max="100" placeholder="Masukkan persentase diskon">
                    @error('persentase')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3 hidden" id="nominal_field">
                    ...
                    <input type="number" class="form-control @error('nominal') is-invalid @enderror" id="nominal" name="nominal" value="{{ old('nominal', $diskon->nominal) }}" step="1000" min="0" placeholder="Masukkan nominal diskon">
                    @error('nominal')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
